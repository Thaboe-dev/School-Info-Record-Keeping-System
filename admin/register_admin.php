...
if (isset($_POST['register'])) {
	...
	if (empty($username)) {
		$input_error['username'] = "The UserName Fied is Required";
	}
	if (empty($password)) {
	...
	if (count($input_error) == 0) {
		$check_email = mysqli_query($db_con, "SELECT * FROM `admins` WHERE `email`='$email';");

		if (mysqli_num_rows($check_email) == 0) {
			$check_username = mysqli_query($db_con, "SELECT * FROM `admins` WHERE `username`='$username';");
			if (mysqli_num_rows($check_username) == 0) {
				if (strlen($username) > 7) {
					if (strlen($password) > 7) {
						$password = sha1(md5($password));
						$query = "INSERT INTO `admins`(`name`, `email`, `username`, `password`, `photo`, `status`) VALUES ('$name', '$email', '$username', '$password','$photo_name','active');";
		...
		<div class="d-flex justify-content-center">
			...
			if (isset($_GET['insert'])) {
				if ($_GET['insert'] == 'sucess') {
					echo '<div role="alert" aria-live="assertive" aria-atomic="true" align="center" class="toast alert alert-success fade hide" data-delay="2000">Your Data Inserted!</div>';
				}
			}; ?>
		</div>
